...
Одностраничное веб-приложение для расчёта операционного финансового цикла (ОФЦ) и связанных коэффициентов ликвидности, рентабельности и устойчивости российских компаний. Данные могут загружаться из Checko API либо из встроенного демонстрационного набора (если нет API-ключа).

## Возможности
- Запрос финансовой отчётности по ИНН и году.
...
- Встроенный демо-режим для офлайн демонстрации и экономии лимита (100 запросов/день на бесплатном тарифе Checko).

## Запуск локально
```bash
# 1) Установите Node.js (v18+). Внешние зависимости не требуются
# 2) (Опционально) создайте .env с ключом Checko
...
- `CHECKO_API_KEY` — ключ Checko API. При его отсутствии автоматически включается демо-режим.
...
3. В разделе Environment Variables добавьте `CHECKO_API_KEY` (и при необходимости `CHECKO_API_BASE`).
4. После деплоя приложение будет доступно на домене Vercel; фронтенд работает из `public/`, API — маршрут `/api/analyze` вашего сервера.
...
- Базовый путь задаётся переменной `CHECKO_API_BASE`. Запрос формируется как `${CHECKO_API_BASE}/{ИНН}/financials?year={год}&key={API_KEY}`. При необходимости скорректируйте путь под актуальную схему Checko.
- Обратите внимание на дневной лимит (100 запросов на бесплатном тарифе). Используйте демо-режим при отладке интерфейса.
