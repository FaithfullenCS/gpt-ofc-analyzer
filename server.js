...
function estimatedRequests(innCount, includePreviousYear) {
  const perCompany = includePreviousYear ? 2 : 1;
  return innCount * perCompany;
}

async function fetchCheckoFinancials(inn, year, options = {}) {
  const { forceMock = false, sections = [] } = options;
...
async function handleAnalyze(req, res) {
  try {
    ...
    const { inns = [], inn, year, previousYear, forceMock, sections = [] } = body;
    ...
    if (!innList.length || !year) {
      respondJson(res, 400, { error: 'Необходимо указать хотя бы один ИНН и год для анализа.' });
      return;
    }

    const prevYear = previousYear || Number(year) - 1;
    const mock = forceMock === true || forceMock === 'true';
    const includePrevious = Boolean(prevYear);
    const neededRequests = mock ? 0 : estimatedRequests(innList.length, includePrevious);
    ...
    const computations = await Promise.all(
      innList.map(async candidateInn => {
        const [currentReport, prevReport] = await Promise.all([
          fetchCheckoFinancials(candidateInn, year, { forceMock: mock, sections: normalizedSections }),
          fetchCheckoFinancials(candidateInn, prevYear, { forceMock: mock, sections: normalizedSections }),
        ]);
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'POST' && parsedUrl.pathname === '/api/estimate') {
    parseBody(req)
      .then(body => {
        const { inns = [], includePreviousYear } = body;
        const innCount = Array.isArray(inns) ? inns.filter(Boolean).length : 0;
        const required = estimatedRequests(innCount, includePreviousYear);
